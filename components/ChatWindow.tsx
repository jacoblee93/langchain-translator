...
export function ChatWindow(props: {
  ...
  async function sendMessage(e: FormEvent<HTMLFormElement> | KeyboardEvent<HTMLTextAreaElement>) {
    e.preventDefault();
    ...
    let accumulatedRawMessage = textDecoder.decode(chunk.value);
    ...
    while (!chunk.done) {
      ...
      accumulatedRawMessage += textDecoder.decode(chunk.value);
      aiResponseMessage.content = accumulatedRawMessage.trim();
      setMessages([...newMessages, aiResponseMessage]);
    }
    ...
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow overflow-hidden ${(messages.length > 0 ? "border" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        <div className="flex w-full mt-4">
          <textarea
            className="grow mr-8 p-4 rounded h-[56px]"
            value={input}
            placeholder={messages.length === 0 ? placeholder : "Ask for edits or ask a follow-up question!"}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.metaKey && e.key === "Enter" && sendMessage(e)}
          />
