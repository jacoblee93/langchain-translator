  ...
  return (
    ...
    >
      <div className="flex">
        ...
        <div className="whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: role === "ai" ? marked.parse(content) : content }}>
        </div>
      </div>
      <div className={`${!traceUrl ? "hidden" : ""} ml-auto mt-2`}>
        <code className="bg-slate-600 px-2 py-1 rounded hover:text-blue-600">
      ...
      <div className={`${(feedback && showCommentForm) ? "" : "hidden"} min-w-[480px]`}>
        <form onSubmit={handleCommentSubmission} className="relative">
          <input
              ...
              value={comment}
              placeholder={feedback?.score === 1 ? "Anything else you'd like to add about this response?" : "What would you have changed about this response?"}
